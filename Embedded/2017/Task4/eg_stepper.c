...
void TaskLED(void *Id)
{
	...
	err = OSCreateTimer( &Tmr2, TmrFunc1, 2, 100, OS_TMR_DISABLE);
	err = OSCreateTimer( &Tmr3, TmrFunc1, 3, 100, OS_TMR_DISABLE);
	...
	for (;;) 
	{
		...
		{
			case 'A': 
				uHALr_printf("EnableTimer1\n");
				OSEnableTimer(Tmr1);
				break;
			case 'B': 
				uHALr_printf("DisableTimer1\n");
				OSDisableTimer(Tmr1);
				OSDisableTimer(Tmr2);
				OSDisableTimer(Tmr3);
				break;
