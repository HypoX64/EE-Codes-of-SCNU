...
#define ETH_DBG   Uart_Printf 
int cs8900a_eth_init (bd_t * bd);
extern int cs8900_e2prom_read(uchar, ushort *);
extern void cs8900_e2prom_write(uchar, ushort);
extern int s3c44b0_eth_init(void);
extern int s3c44b0_eth_send(unsigned char *data, int len);
extern int s3c44b0_eth_rcv(unsigned char *data, int *len);
extern int s3c44b0_eth_get_address(unsigned char *address);

#endif // #define	__S3C44B0_H
